...
{
    std::fstream stream_data(filepath + ".data");
    std::fstream stream_label(filepath + ".labels");

    std::string line;

    while(getline(stream_data, line))
    {
        ...
        {
            features.push_back(std::stoi(s));
        }

        int label;
        stream_label >> label;

        objects_.push_back(Object(((label == 1) ? 1 : 0), features));
    }
